...
1. **Убедиться что MCP Proxy запущен** — проверить `http://localhost:3100/health`. Если нет — попросить пользователя запустить `npm start` в терминале.
2. **Убедиться что JS-агент подключен** — проверить что `wsClients > 0` в ответе health. Если нет — попросить пользователя открыть консоль браузера (F12) на целевом сайте и вставить код из `dist/js-agent-bundle.js`.
3. **Создать папку исследования** — `study/<project-name>/` (имя проекта на основе URL сайта, например `study/example-com/`).
4. **Начать сбор данных** через MCP-инструменты.
